...
public class MyAnimator extends Canvas { // declaration of MyCanvas class which extends 'Canvas' class to create a custom
                                       // canvas for drawing.
    ...
    public MyAnimator(VisualizerProvider listener) { // Constructor for MyCanvas class which initializes the canvas with
                                               // specified listener.
    ...
    public void clear(Graphics graphic) { // Custom method to clear the Canvas
        graphic.setColor(ColorPicker.CANVAS_BACKGROUND); // sets the color of the graphics context to the canvas
                                                          // background color.
        graphic.fillRect(0, 0, getWidth(), getHeight()); // Draws a filled rectangle representing the
                                                         // canvas with the background color.
    }

    public interface VisualizerProvider { // Defines the VisualProvider interface which has a single method 'onDrawArray'
